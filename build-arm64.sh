...
DEBIAN_INSTALLER_PATH=main/installer-${ARCH}/current/images/netboot/debian-installer
...
if [ ${result} -eq 0 ]; then
    if [ ! -e ${WORKDIR}/initrd.gz ] || [ ! -e ${WORKDIR}/linux ]; then
        ...
        url=${DEBIAN_BASE_URL}/${DEBIAN_VERSION}/${DEBIAN_INSTALLER_PATH}/${ARCH}
        mkdir -p ${WORKDIR} && \
        pushd ${WORKDIR} >/dev/null && \
        wget -qc ${url}/initrd.gz && \
        wget -qc ${url}/linux && \
        popd >/dev/null
        result=${?}
    fi
...
if [ ${result} -eq 0 ]; then
    if [ ! -e ${DISK_PATH}/install.done ]; then
        ...
        ${QEMU} \
            -smp ${CORES} -M virt -cpu ${CPU} -m ${MEM} \
            -initrd ${WORKDIR}/initrd.gz -kernel ${WORKDIR}/linux \
            -append "${bootcmd}" \
            \
